...
def extract(logger, m, loader, discrete, norm_feats, device):
    z, y, y_emb, labels = extract_features(m, loader, device, discrete)
...
def execute(args):
    ...
    z, y, y_emb, labels = extract(logger, m, loaders['train'], discrete, parameters['norm_feats'], args.device)
    ...
    classifiers_on_y_emb = train_classifier(y_emb.cpu().numpy(), labels, attribute_info, ridge=args.ridge, solver='auto')

    logger.exp.info('Evaluating at val')
    logger.exp.info(f'Extracting features on set val')
    z, y, y_emb, labels = extract(logger, m, loaders['val'], discrete, parameters['norm_feats'], args.device)
    ...
    eval_stats = evaluate(logger, score_df, y_emb, labels, classifiers_on_y_emb, 'val', 'y_emb', attribute_info, args.device)

    logger.exp.info('Evaluating at test ood')
    logger.exp.info(f'Extracting features on set test')
    z, y, y_emb, labels = extract(logger, m, loaders['ood'], discrete, parameters['norm_feats'], args.device)
    ...
    eval_stats = evaluate(logger, score_df, y_emb, labels, classifiers_on_y_emb, 'ood', 'y_emb', attribute_info, args.device)

    logger.exp.info('Extracting features on set test iid')
    z, y, y_emb, labels = extract(logger, m, loaders['iid'], discrete, parameters['norm_feats'], args.device)
    ...
    eval_stats = evaluate(logger, score_df, y_emb, labels, classifiers_on_y_emb, 'iid', 'y_emb', attribute_info, args.device)
